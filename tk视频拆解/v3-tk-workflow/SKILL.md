...
**每次启动本 skill，无论用户说了什么，必须先问清以下信息，再开始采集。**

### 必问 Q1 — 分析模式

> "这次拆解用哪个模式？"
> A. **竞品对比模式**（默认推荐）——找 Top 10 竞品视频 + 我方视频，生成矩阵对比报告，和 Curvlife 指甲胶报告格式一样
> B. **单视频拆解**——只拆一条视频，生成深度拆解报告（6个步骤详析）

### 必问 Q2 — 我方视频/达人

> "有没有「我方」视频或达人账号？（竞品对比模式中「我方」列的数据来源）"
> A. 有 → 请粘贴 TikTok 视频链接或 @达人账号名
> B. 没有 → 报告不设「我方」列，做纯竞品 TOP10 分析
> C. 自己去 FastMoss 找 → 我会从 FastMoss 选最具代表性的一条作参考

### 必问 Q3 — 商品/品类

> "要分析哪个商品或品类？"
> A. 指定商品 → 请告知商品名或 TikTok Shop 链接（我会用 product_search 找到 product_id）
> B. 指定品类关键词 → 告知关键词（如"香水""指甲胶""护肤"），我用 video_search 发现 Top 视频

### 可选 Q4 — 时间窗口和筛选条件（不问默认 90 天）

> "有没有额外筛选条件？比如：只看自然流量 / 粉丝数限制 / GMV 门槛 / 发布时间范围？"
> 不说就按默认：90 天窗口，不限投流，按 GMV 降序 Top 10。
...
1. **分析模式**：竞品对比（TOP10矩阵，推荐）还是单视频深度拆解？
2. **我方视频**：有没有你们自己的视频链接？（没有可以不设「我方」列）
3. **目标商品/品类**：要分析哪款商品或哪个品类关键词？
4. **筛选条件**（可选）：比如只看自然流量、粉丝上限、GMV门槛等？
```

---

## ★ 执行总纲（确认反问答案后，按这9步走，不要跳步）

```
① 入口反问      ← 见上方，必须先执行
② 选品          近90天（默认）+ 筛选条件，不足10条按 90→28→14→7 天升级，报告注明实际窗口
...
区块2: 10条竞品逐条拆解（卡片折叠式，每条可展开）
  ...
  ├── ⚠️【强制，缺一张=报告不合格】底部元数据条（每张卡片必须有，紧接评分下方）:
  ...
  └── ⚠️【强制，缺失=报告不合格】底部共性小结（仅在最后一张卡片之后）:
...
区块4: 行动方案（★原「行动清单」+「方案建议」合并）
  ...
  │     ├── CTA话术（原文，含购物车引导方式）
  │     └── ⚠️【强制中英双语】末尾附2个可直接拍的完整脚本成稿（分时间段，对标不同竞品）
  │           每个 beat 三行格式：
  │           [时间戳][段落名]
  │           EN: "完整英文口语句，达人直接念出口"  ← 主体，不是关键词罗列
  │           CN: 中文含义（括号内写导演拍摄提示，如「手部特写」「快切」）
  │
  ├── 4.2 视频怎么拍
  │     ├── 画面配比（特写/口播/全景各占比）
  │     ├── 剪辑节奏（每几秒切一次）
...
## 单视频HTML报告：直接生成独立HTML

> ⚠️ **视觉规范已升级为竞品分析风格（2026-09）**
> 不再使用 `inject_report.py + template.html`（旧版暖棕色调）。
> 所有单视频拆解报告必须直接生成独立 HTML 文件，使用以下视觉规范：
>
> **字体**：Inter（正文/数字）· JetBrains Mono（所有数字/代码）· Syne（标题/数字大字）
> **颜色**：`--blue: #2563EB` · `--orange: #F97316` · `--purple: #7C3AED` · `--green: #059669` · `--red: #DC2626`
> **背景**：`--bg: #FFFFFF` · `--bg-alt: #F8FAFC` · `--border: #E5E7EB`
> **布局**：卡片式，圆角12px，max-width 960px，16px侧间距，深色模式双主题
> **参考文件**：`video_perfume/report_honeyoud_v2.html`（此文件是视觉规范基准）
>
> 每次生成报告时，直接写 HTML，不要调用 inject_report.py，不要用 template.html。
...
**生成命令**：直接用 Write 工具写出独立 HTML 文件（无需 inject_report.py）：

```
Write: videoX/report_videoX.html
内容: 完整 HTML，包含内联 CSS（竞品分析视觉规范）+ 所有报告内容
```

**结构模板**（必须包含以下区块，顺序固定）：
```
Hero（渐变背景，5个大数字，meta 标签）
FastMoss 商业面板（GMV · Lift · 互动率 · 产品信息）
区块01 视频深度扫描（帧时间线表格 + 5维评分条）
解读卡 ×3（关键发现 · 观看深度解读 · 发布时段分析）
区块02 爆款诊断（Lift对比 + 趋势时间线）
区块03 脚本结构拆解（病毒评分 + 5段 + 硬规则 + 修复建议）
区块04 买家评论洞察（评分分布 + 典型评论 + 信号卡）
区块05 复制公式引擎（BEND判定 + 角色 + 框架 + 可复用表）
结论（渐变背景卡）
```
...
- [ ] ⛔ **【硬性阻断】区块02每张竞品卡片是否有底部元数据条？** — 必须包含：① Caption原文、② 标题策略分析（1句）、③ 话题标签色块（全部 #xxx）。10张卡片全部必须有，一张都不能少，否则停止交付，补全后再发。
- [ ] ⛔ **【硬性阻断】区块02末尾（#10卡片之后）是否有共性小结 + 话题标签分析卡片？** — 共性小结含时长规律/展示方式/GMV分布；话题标签分析含5格子分类（核心词/高GMV信号词/解决方案词/大众流量词/品牌词）+ 纠错框。缺失则停止交付。
...
- [ ] ⛔ **【硬性阻断】两版成稿是否都是中英双语？** — 每个 beat 必须三行：`[时间戳][段落名]` / `EN: "完整英文口语句"` / `CN: 中文含义（含导演拍摄提示）`。任何一版只有中文、或 EN 是关键词罗列而非完整可念句子，停止交付，补全后再发。
- [ ] 两版对标不同竞品打法，不雷同
- [ ] 各 beat 时间戳加总 = 目标时长
