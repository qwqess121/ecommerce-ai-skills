...
description: "Build or optimize Amazon listings: create from keywords, or audit keyword gaps and score 8 dimensions. Use when creating, auditing, or rewriting a listing."
...
### SIF（sif-mcp）
- `ops_get_listing_traffic_overview` → 获取当前 listing 的流量数据（session、转化率、关键词来源），评估优化前基线
- `ops_get_listing_keyword_distribution` → 获取关键词分布和自然排名，识别已有覆盖和缺失的关键词

### 卖家精灵（237a35ba）
- `keyword_research` → 查询目标关键词的搜索量、竞争度，为关键词分层提供数据支撑

### Brightdata
- `scrape` → 抓取竞品 Listing 全文（标题、五点、描述、A+），做关键词提取和对比分析

### 降级方案
如果 MCP 工具不可用，回退到原有的 web_search / fetch-listing.sh / 用户手动输入方式。
