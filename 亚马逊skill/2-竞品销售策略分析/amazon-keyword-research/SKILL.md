...
### 卖家精灵（237a35ba）
- `keyword_miner` → 扩展关键词来源（替代部分 autocomplete）
- `keyword_research` → 搜索量/竞争度数据
- `google_trend` → 替代 web_fetch Google Trends

### SIF（sif-mcp）
- `market_get_keyword_demand` → ABA 级别搜索量

### 补充说明
保留 `research.sh` autocomplete 脚本作为补充数据源，与 MCP 工具互为验证。

### 降级方案
如果 MCP 工具不可用，回退到原有的 research.sh autocomplete 脚本 + web_search / web_fetch 方式。
