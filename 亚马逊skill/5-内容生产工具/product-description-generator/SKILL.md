...
### 卖家精灵（237a35ba）
- `keyword_research` → 获取关键词搜索量、竞争度和相关词数据，替代 web_search 进行关键词扩展和优先级评分

### Brightdata
- `scrape` → 抓取竞品 Listing 页面完整内容（标题、Bullet Points、描述、价格、评论数），替代 web_fetch 更稳定可靠

### 降级方案
如果 MCP 工具不可用，回退到 web_fetch / web_search 等原有方式采集竞品数据和关键词。
