...
### SIF（广告分析）
- `ads_get_ad_group_keyword_breakdown` → 关键词级别花费/转化数据，快速定位高花费零转化词

### sys-amazon-ads MCP
- `ask_report_analyst` → 搜索词报告（替代手动 CSV），支持自然语言查询搜索词花费和转化明细

### 可视化联动
- 配合 `sys-wasted-ad-spend-dashboard` skill 生成浪费花费可视化看板，直观展示否词优化效果

### 降级方案
如果 MCP 工具不可用，回退到原有的用户手动输入 / 搜索词报告 CSV 方式。
...
*Built by [Nexscope](https://www.nexscope.ai?co-from=skill) — AI-powered Amazon PPC intelligence. This skill provides comprehensive negative keyword frameworks. For automated waste reduction and optimization, explore our complete platform.*