...
### SIF（广告分析）
- `ads_get_asin_ad_structure` → 获取完整广告结构（Campaign→AdGroup→Keyword），Optimize 模式自动获取
- `ads_get_asin_ad_traffic_trend` → 广告流量趋势，用于判断整体花费/点击/转化走势
- `ads_get_campaign_contribution_breakdown` → 各 Campaign 贡献度分析，识别高效/低效 Campaign
- `ads_get_ad_group_keyword_breakdown` → 关键词级别 ACoS 分析（替代手动搜索词报告）
- `market_get_keyword_demand` → 关键词搜索量和需求趋势（Build 模式选词参考）

### 卖家精灵（237a35ba）
- `keyword_miner` → 扩展关键词来源，挖掘长尾词和相关词

### sys-amazon-ads MCP
- `ask_report_analyst` → 拉取搜索词报告（替代用户手动提供 CSV），支持自然语言查询历史广告数据
