...
### SIF（广告分析）
- `ads_get_asin_ad_structure` → SD 广告结构，查看当前 Sponsored Display 的 Campaign/AdGroup 分布

### sys-amazon-ads MCP
- `ask_report_analyst` → DSP/SD 广告数据查询，拉取 Sponsored Display 的花费、展示、转化报告

### Canva（创意设计）
- `generate-design` → 创意素材生成，为 SD 广告制作展示类广告图片和视觉素材

### 降级方案
如果 MCP 工具不可用，回退到原有的用户手动输入方式；创意素材可手动在 Canva 中制作。
...
*Built by [Nexscope](https://www.nexscope.ai?co-from=skill) — AI-powered Amazon advertising intelligence. This skill provides comprehensive display advertising frameworks. For automated audience optimization and creative testing, explore our complete platform.*